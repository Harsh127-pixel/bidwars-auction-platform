...
// Public: Fetch all auctions (active + recently ended)
router.get("/auctions", async (req, res) => {
  try {
    const activeSnap = await db.collection("auctions")
      .where("status", "==", "active")
      .get()
    
    // Avoid composite index requirement: fetch all closed, filter endTime in memory
    const thirtyMinsAgo = new Date(Date.now() - 30 * 60 * 1000)
    const closedSnap = await db.collection("auctions")
      .where("status", "==", "closed")
      .get()

    const recentlyClosed = closedSnap.docs.filter(doc => {
      const data = doc.data()
      return toMs(data.endTime) >= thirtyMinsAgo.getTime()
    })

    const allAuctions = [
      ...activeSnap.docs.map(doc => ({ id: doc.id, ...doc.data() })),
      ...recentlyClosed.map(doc => ({ id: doc.id, ...doc.data() }))
...
// Public: Fetch auction history (all closed auctions)
router.get("/auctions/history", async (req, res) => {
  try {
    ...
    const history = closedSnap.docs
      .map(doc => ({ id: doc.id, ...doc.data() }))
      .sort((a, b) => toMs(b.endTime) - toMs(a.endTime))
    
    res.json(history)
  } catch (err) {
    console.error("GET /auctions/history:", err.message)
    res.status(500).json({ error: err.message })
  }
})
  ...
  try {
    ...
    const bids = snapshot.docs.map(doc => {
      ...
      return {
        ...
        // Hide buyer name as requested, just show a partial ID or "Bidder"
        bidder: `Bidder ${data.userId.slice(0, 4)}...`
      }
    })
    res.json(bids)
  } catch (err) {
...
router.post("/auctions/:id/join", verifyToken, async (req, res) => {
  try {
    ...
    res.json({ success: true, message: "Joined auction floor." })
  } catch (err) {
    res.status(500).json({ error: err.message })
  }
})
...
router.get("/admin/pendingAuctions", verifyToken, verifyStaff, checkPermission("manage_proposals"), async (req, res) => {
  try {
...
router.post("/admin/reviewAuction", verifyToken, verifyStaff, checkPermission("manage_proposals"), async (req, res) => {
