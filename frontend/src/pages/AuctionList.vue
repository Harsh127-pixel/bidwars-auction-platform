...
const closedAuctions = ref([])
const fetchHistory = async () => {
  loading.value = true
  try {
    const res = await api.get('/api/auctions/history')
    closedAuctions.value = res.data
  } catch { notification.add('Failed to load history', 'error') }
  finally { loading.value = false }
}
...
const filtered = computed(() => {
  if (category.value === 'History') return closedAuctions.value
...
const handleCategoryChange = (cat) => {
  ...
  else if (auctions.value.length === 0) fetchAuctions()
}
  ...
  <div class="list-page">
    ...
    <div class="page-wrap list-body">
      ...
      <div class="filter-bar fade-up">
        <!-- Category pills -->
        <div class="cat-pills">
          <button v-for="cat in [...categories, 'History']" :key="cat"
            class="cat-pill" :class="{'cat-pill--on': category === cat, 'cat-pill--history': cat === 'History'}"
        ...
        <div class="filter-controls">
          ...
          <button class="view-btn" @click="fetchAuctions" :disabled="loading" title="Refresh">
      ...
      <TransitionGroup v-else-if="filtered.length"
        ...
        <AuctionCard
          ...
          class="fade-up"
          :class="{ 'card-closed': category === 'History' }"
...
.cat-pill--history { border-color: var(--gold-border); }
.cat-pill--history.cat-pill--on { background: var(--gold-dim); color: var(--gold); }

.card-closed { opacity: 0.85; filter: grayscale(0.2); }
