...
const isPro = computed(() => authStore.user?.subscriptionStatus === 'active')
const tier = computed(() => {
  if (isPro.value) {
...
const commissionRate = computed(() => {
  ...
  return { Gold: 0, Silver: 3, Bronze: 5 }[tier.value] || 5
})
...
const subscribe = async (plan) => {
  try {
    await api.post('/api/users/subscription', { tier: plan, duration: 'monthly' }) 
...
const unsubscribe = async () => {
  try {
    if (!confirm('Are you sure you want to cancel your Pro membership?')) return
      ...
      <div class="profile-hero fade-up">
        ...
        <div class="profile-hero__inner">
          ...
          <div class="profile-hero__info">
            <div class="hero-username" :class="{ 'hero-username--pro': isPro }">
          ...
          <div class="profile-hero__stats">
            <div class="hero-stat">
              <div class="hero-stat__val">{{ authStore.user?.totalWins || wonItems.length }}</div>
...
.hero-username { font-family: var(--font-display); font-size: clamp(22px, 3vw, 32px); color: var(--text); margin-bottom: 4px; display: flex; align-items: center; gap: 10px; }
.hero-username--pro {
