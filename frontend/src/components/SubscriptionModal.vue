...
const paymentMethod = ref('wallet') // Default to wallet
...
const balance = computed(() => authStore.user?.credits || 0)
const hasEnoughBalance = computed(() => {
  if (!selectedPlan.value) return false
  return balance.value >= selectedPlan.value.price
})
...
const plans = [
  {
    ...
    icon: '🥈',
    features: [
      { text: '3% Platform fee (regular 5%)', highlight: true },
      { text: 'Standard priority bids', highlight: false },
      { text: 'Basic auction analytics', highlight: false },
  ...
  },
  {
    ...
    icon: '🥇',
    features: [
      { text: '0% Platform fee (save all)', highlight: true },
      { text: 'High priority bid processing', highlight: true },
      { text: 'Advanced auction analytics', highlight: true },
      { text: 'Early access to premium lots', highlight: false },
      { text: 'VIP Support access', highlight: false },
    ]
  }
]

const selectedDuration = ref('monthly') // 'monthly' | 'yearly'
...
const selectPlan = (plan) => {
  ...
  const price = selectedDuration.value === 'monthly' ? plan.price_monthly : plan.price_yearly
  
  // Always use wallet as requested by user
...
const processPayment = async () => {
  ...
  try {
    const res = await api.post('/api/users/subscription', {
      tier: selectedPlan.value.id.charAt(0).toUpperCase() + selectedPlan.value.id.slice(1),
      duration: selectedDuration.value,
      paymentMethod: 'wallet'
    })
    
    // Update local store with data from response or optimistic
    if (authStore.user) {
       await authStore.fetchProfile() // Refresh profile from server to be sure
    }
  } catch (e) {
    paymentError.value = e.response?.data?.error || 'Deduction from wallet failed.'
    step.value = 'payment'
    return
  }
        ...
        <div class="sub-modal" :class="`step-${step}`">
          ...
          <div v-if="step === 'plans'" class="step-plans">
            ...
            <div v-else>
              ...
              <div class="duration-toggle">
                <button :class="{ active: selectedDuration === 'monthly' }" @click="selectedDuration = 'monthly'">Monthly</button>
                <button :class="{ active: selectedDuration === 'yearly' }" @click="selectedDuration = 'yearly'">Yearly <span>Save ~25%</span></button>
              </div>
              ...
              <div class="plan-cards">
                <div v-for="plan in plans" :key="plan.id"
                  class="plan-card"
                  :class="{ 'plan-card--featured': plan.id === 'gold' }"
                  @click="selectPlan(plan)">
                  <div class="plan-icon">{{ plan.icon }}</div>
                  <div class="plan-name">{{ plan.name }}</div>
                  <div class="plan-price-group">
                    <span class="plan-currency">₹</span>
                    <span class="plan-price">{{ (selectedDuration === 'monthly' ? plan.price_monthly : plan.price_yearly / 12).toFixed(0) }}</span>
                    <span class="plan-period">/mo</span>
                  </div>
                  <div class="plan-sub-price" v-if="selectedDuration === 'yearly'">
              ...
              <div class="plans-footer">
                <span>💰 Deducted from Wallet Credits</span>
                <span>·</span>
          ...
          <div v-else-if="step === 'payment'" class="step-payment">
            ...
            <div class="pay-header">
              <div class="pay-plan-badge" :style="`color:${selectedPlan.color}; background:${selectedPlan.color}20` ">{{ selectedPlan.icon }} {{ selectedPlan.name }}</div>
              <div class="pay-amount">₹{{ (selectedDuration === 'monthly' ? selectedPlan.price_monthly : selectedPlan.price_yearly).toLocaleString('en-IN') }}<span>/{{ selectedDuration }}</span></div>
              <div class="pay-subtitle">Payment via Wallet Credits</div>
              <div class="wallet-balance mt-2">
                <span class="t-label">Wallet Balance:</span> 
                <span :style="`color: ${authStore.user?.credits < (selectedDuration === 'monthly' ? selectedPlan.price_monthly : selectedPlan.price_yearly) ? 'var(--red)' : 'var(--green)'}`" style="font-weight:700;margin-left:8px">
                  ₹{{ (authStore.user?.credits || 0).toLocaleString('en-IN') }}
                </span>
              </div>
            </div>

            <!-- Payment method selection removed as we use Wallet -->
            <div class="pay-form">
               <div style="background:var(--bg-raised);padding:16px;border-radius:12px;text-align:center;font-size:14px;color:var(--text-2)">
                  The subscription amount will be deducted directly from your BidWars wallet.
               </div>
            </div>

            <div v-if="paymentError" class="pay-error">⚠ {{ paymentError }}</div>

            <div v-if="authStore.user?.credits < (selectedDuration === 'monthly' ? selectedPlan.price_monthly : selectedPlan.price_yearly)" class="wallet-upsell">
               <div class="upsell-msg">Your wallet balance is low. Please add funds to continue.</div>
               <button class="btn-topup-suggest" @click="goToWallet">Add Money to Wallet</button>
            </div>

            <button v-else class="pay-submit" @click="processPayment">
              Confirm & Deduct ₹{{ (selectedDuration === 'monthly' ? selectedPlan.price_monthly : selectedPlan.price_yearly).toLocaleString('en-IN') }}
            </button>
          ...
          <div v-else-if="step === 'processing'" class="step-processing">
            ...
            <div class="processing-title">Processing Wallet Deduction</div>
            <div class="processing-sub">Securing your credits and activating tier…</div>
            ...
            <div class="success-perks">
              ...
              <div class="success-perk">
                ...
                <div>
                  ...
                  <div class="success-perk__val">{{ selectedPlan?.name }} unlocked</div>
                  <div class="success-perk__sub" style="font-size:10px;color:var(--text-3)">{{ selectedDuration.toUpperCase() }} PLAN</div>
                </div>
              </div>
            </div>
...
.balance-status__icon {
  font-size: 32px;
}

.balance-status__info {
  flex: 1;
}
...
.duration-toggle {
  ...
  background: var(--bg-raised); border: 1px solid var(--border);
  border-radius: 12px; padding: 4px; max-width: 320px; margin-left: auto; margin-right: auto;
}
...
.duration-toggle button span { font-size: 10px; color: var(--green); display: block; }
.duration-toggle button.active { background: var(--bg-card); color: var(--gold); box-shadow: var(--shadow-sm); }

.plan-sub-price { font-size: 12px; color: var(--text-3); margin-top: -14px; margin-bottom: 20px; }
