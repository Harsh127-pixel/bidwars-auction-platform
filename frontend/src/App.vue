  ...
  import('./services/socket').then(({ default: socket }) => {
    socket.on('bidUpdate', (data) => {
      if (data.previousBidder === authStore.user?.uid && data.userId !== authStore.user?.uid) {
        notifications.value.unshift({
          ...
          message: `Someone just bid ₹${data.amount.toLocaleString()}`,
          createdAt: new Date(),
          icon: 'mdi-gavel',
          type: 'outbid',
          read: false
    ...
    socket.on('biddingStopped', (data) => {
      notifications.value.unshift({
        ...
        message: 'Click to see final results',
        createdAt: new Date(),
          ...
          <v-menu :close-on-content-click="false">
            ...
            <v-list width="320" class="rounded-xl pa-0 mt-2"
              ...
              <v-list-item v-for="n in notifications" :key="n.id" class="py-3 px-4 border-b">
                <template v-slot:prepend>
                  <v-avatar :color="n.type === 'outbid' || n.type === 'OUTBID' ? 'error' : 'info'" variant="tonal" size="36"
